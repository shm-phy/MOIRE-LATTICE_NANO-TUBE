...
def write_data(theta, Lx_sup, Ly_sup, Nx, Ny, H, v, Cz, \
        layer1, cnt1, bond1, b_c1, layer2, cnt2, bond2, b_c2, bond_flag):

    directory = "./LAMMPS_DATA_BOND/Angle_"+str("{:.1f}".format(theta))+"/"
    filename = "data.Gr_rect_"+str(int(Lx_sup*Nx/10))+"_"+str(int(Ly_sup*Ny/10))
...
if __name__ == "__main__" :
    ...
    if cnt1 == cnt2 and write_flag:
        write_data(theta, Lx_sup, Ly_sup, Nx, Ny, H, vertices, Cz, \
             layer1, cnt1, bond1, b_c1, layer2, cnt2, bond2, b_c2, bond_flag)
