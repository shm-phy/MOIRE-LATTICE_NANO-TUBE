...
def cs_theta(m, r, lx):
    ...
    print("\tLattice constant of hexagonal moire unit cell: ", distance.euclidean(vertices[0, :], vertices[1, :])/10, "nm")
...
def create_layer2(layer1, m, r, H, v):
    ...
    return layer1, layer2, v, unit, Lx_sup, Ly_sup
...
inside the script (line no. 524 --> 527)")
        print()
...
def write_data(theta, Lx_sup, Ly_sup, Nx, Ny, H, v, Zz_down, Zz_up, \
        layer1, cnt1, bond1, b_c1, layer2, cnt2, bond2, b_c2, bond_flag):

    directory = "./LAMMPS_DATA/Angle_"+str("{:.1f}".format(theta))+"/"
    filename = "data.MoS2_rect_"+str(int(Lx_sup*Nx/10))+"_"+str(int(Ly_sup*Ny/10))
...
if __name__ == "__main__" :
    ...
    layer1, layer2, vertices, unit ,Lx_sup, Ly_sup = create_layer2(layer1, m, r, H, vertices)
    ...
    if cnt1 == cnt2 and write_flag:
        write_data(theta, Lx_sup, Ly_sup, Nx, Ny, H, vertices, Zz_down, Zz_up, \
             layer1, cnt1, bond1, b_c1, layer2, cnt2, bond2, b_c2, bond_flag)
